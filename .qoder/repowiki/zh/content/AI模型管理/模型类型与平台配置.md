...
## 更新摘要
**变更内容**
- 平台枚举大幅简化：从原有的20多个平台减少到仅4个核心平台
- 国内平台：通义千问(TongYi)、DeepSeek
- 国外平台：OpenAI、Ollama
- 移除了大量已删除平台的相关说明和配置
- 更新了工厂实现中的平台分支和构建方法
...
- AiPlatformEnum 枚举的定义与平台标识符管理：经过平台整合后的简化版平台标识与校验机制。
...
- [AiAutoConfiguration.java:32-67](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiAutoConfiguration.java#L32-L67)
- [AiProperties.java:9-25](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiProperties.java#L9-L25)
- [AiModelSaveReqVO.java:14-60](file://src/main/java/cn/boss/data/ai/controller/model/vo/model/AiModelSaveReqVO.java#L14-L60)
- [AiModelDO.java:21-60](file://src/main/java/cn/boss/data/ai/dal/dataobject/model/AiModelDO.java#L21-L60)
...
- 平台枚举 AiPlatformEnum：经过平台整合后的简化版枚举，统一国内/国外平台标识，提供平台码与名称，内置 validatePlatform 校验方法与可遍历数组。
...
- [AiModelFactoryImpl.java:74-108](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L74-L108)
- [AiAutoConfiguration.java:35-67](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiAutoConfiguration.java#L35-L67)
- [AiProperties.java:9-25](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiProperties.java#L9-L25)
...
### 平台枚举 AiPlatformEnum
- 设计要点
  - 经过平台整合后的简化版枚举，国内平台与国外平台分组，统一平台码与名称，便于跨环境适配。
  - 提供 validatePlatform 方法，集中处理非法平台异常。
  - 实现 ArrayValuable<String>，支持基于平台码的校验与过滤。
- 平台标识符管理
  - **更新** 国内平台：通义千问(TONG_YI)、DeepSeek(DEEP_SEEK)
  - **更新** 国外平台：OpenAI(OPENAI)、Ollama(OLLAMA)
- 兼容性检查机制
...
- 工厂实现设计
  ...
  - **更新** 针对简化后的4个平台提供专用构建方法：通义千问、DeepSeek、OpenAI、Ollama。
...
**图表来源**
- [AiModelFactory.java:13-62](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactory.java#L13-L62)
- [AiModelFactoryImpl.java:71-324](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L71-L324)
...
- 自动装配
  - 注册 AiModelFactory Bean，提供默认模型、温度、最大上下文等参数的默认值设置。
- 配置属性
  - 以 boss.ai.* 为前缀，网络搜索配置，包含启用开关、API Key等。
  - application.yml 中提供示例配置，便于快速启动与调试。

```mermaid
flowchart TD
Start(["加载配置"]) --> BuildBean["构建 AiModelFactory Bean"]
...
- [AiAutoConfiguration.java:35-67](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiAutoConfiguration.java#L35-L67)
- [AiProperties.java:9-25](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiProperties.java#L9-L25)
- [application.yml:100-110](file://src/main/resources/application.yml#L100-L110)
...
- [InEnum.java:22-32](file://src/main/java/cn/boss/data/ai/framework/common/validation/InEnum.java#L22-L32)
- [InEnumValidator.java:11-39](file://src/main/java/cn/boss/data/ai/framework/common/validation/InEnumValidator.java#L11-L39)
- [AiModelSaveReqVO.java:33](file://src/main/java/cn/boss/data/ai/controller/model/vo/model/AiModelSaveReqVO.java#L33)
...
- 配置与平台
  - AiAutoConfiguration 读取 AiProperties 中的配置，为工厂与客户端提供默认参数。
...
**章节来源**
- [AiModelFactoryImpl:147-152](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L147-L152)
- [AiAutoConfiguration:35-67](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiAutoConfiguration.java#L35-L67)
- [AiModelFactoryImpl:223-242](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L223-L242)

## 故障排查指南
- 平台非法
  - 现象：抛出非法平台异常。
  - 排查：确认 AiPlatformEnum.validatePlatform 输入值是否在简化后的4个平台范围内；检查配置文件与请求参数大小写与拼写。
- 模型未启用或禁用
  - 现象：获取模型失败或返回默认模型不存在。
  - 排查：检查 AiModelDO.status 是否为启用状态。
- 配置缺失
  - 现象：默认模型为空或参数未生效。
  - 排查：核对 application.yml 中 boss.ai.web-search 下的 enable、api-key 等字段。
- 向量存储初始化失败
  - 现象：Redis/Qdrant 初始化报错或无法写入。
  - 排查：确认 Redis/Qdrant 地址、端口、认证信息与索引/集合名称正确；检查网络连通性与权限。

**章节来源**
- [AiPlatformEnum.java:39-46](file://src/main/java/cn/boss/data/ai/enums/model/AiPlatformEnum.java#L39-L46)
...
- [application.yml:134-139](file://src/main/resources/application.yml#L134-L139)

## 结论
本方案通过强类型的模型类型与平台枚举，配合工厂与自动装配机制，实现了对精简后4个核心平台、多模型的统一抽象与高效实例化。参数校验与默认配置保障了系统的健壮性与易用性。通过本文档提供的扩展与迁移建议，可在不破坏现有兼容性的前提下平滑引入新类型与新平台。
