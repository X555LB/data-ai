...
## 更新摘要
**所做更改**
- 更新了工厂实现部分，反映AiModelFactoryImpl中平台构建方法的重大精简
- 移除了YiYan、DouBao、AzureOpenAI、Anthropic、Gemini等平台的构建方法
- 更新了支持的平台列表，目前仅保留TongYi、DeepSeek、OpenAI、Ollama四个平台
- 更新了架构图和依赖关系分析，反映当前实际的平台支持情况
- 修正了平台实现示例，删除不再存在的平台类引用
...
**更新** 当前工厂实现已大幅简化，移除了多个平台的支持，目前仅支持TongYi、DeepSeek、OpenAI、Ollama四个平台。
...
subgraph "平台实现"
TY["DashScopeChatModel"]
DS["DeepSeekChatModel"]
OA["OpenAiChatModel"]
OL["OllamaChatModel"]
end
SVC --> IF
...
- 工厂实现：集中处理平台识别、参数构建、实例创建与缓存，屏蔽平台差异。**更新** 当前实现已简化，仅支持4个平台。
- 平台枚举：标准化平台标识，便于工厂分支选择与校验。**更新** 当前枚举包含TongYi、DeepSeek、OpenAI、Ollama四个平台。
...
- [AiAutoConfiguration.java:34-59](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiAutoConfiguration.java#L34-L59)
- [AiProperties.java:11-25](file://src/main/java/cn/boss/data/ai/framework/ai/config/AiProperties.java#L11-L25)
...
alt Spring Bean获取
Factory->>Spring : 直接获取平台Bean
Spring-->>Factory : 返回平台客户端
else 直接构建
...
SwitchPlatform --> BeanOrDirect["Spring Bean获取<br/>或直接构造"]
...
**图表来源**
- [AiModelFactoryImpl.java:147-152](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L147-L152)
- [AiModelFactoryImpl.java:74-108](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L74-L108)
...
### 平台枚举与类型枚举
- 平台枚举：统一管理国内外主流AI平台，提供平台标识与名称映射，支持校验与数组转换。**更新** 当前支持TongYi、DeepSeek、OpenAI、Ollama四个平台。
...
### 平台实现类示例
**更新** 当前实现已移除百川、豆包、Gemini等平台，仅保留以下平台实现：

- DashScopeChatModel：通义千问平台的Spring AI实现
- DeepSeekChatModel：DeepSeek平台的Spring AI实现  
- OpenAiChatModel：OpenAI官方平台的Spring AI实现
- OllamaChatModel：本地Ollama平台的Spring AI实现

通过常量定义默认URL与模型，便于快速集成与替换。

**章节来源**
- [AiModelFactoryImpl.java:156-193](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L156-L193)
...
-buildTongYiEmbeddingModel(apiKey, model)
-buildOpenAiEmbeddingModel(token, url, model)
-buildOllamaEmbeddingModel(url, model)
...
## 故障排查指南
- 平台未识别：检查AiPlatformEnum是否包含目标平台，或确认传入平台字符串是否正确。**更新** 当前支持的平台为TongYi、DeepSeek、OpenAI、Ollama。
- 密钥格式错误：部分平台（如通义千问、DeepSeek）要求特定格式的密钥组合，需按实现要求提供。
...
**更新** 当前实现已简化为支持4个主要平台，提高了系统的稳定性和可维护性。

## 附录

### 扩展新AI平台步骤
- 在平台枚举中添加新平台标识与名称。**更新** 当前枚举已简化，新增平台需要谨慎评估。
...
**章节来源**
- [AiModelFactoryImpl.java:147-152](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L147-L152)
- [AiModelFactoryImpl.java:221-242](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L221-L242)