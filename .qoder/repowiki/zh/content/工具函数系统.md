...
## 更新摘要
**变更内容**
- 新增JsonUtils类作为核心JSON处理工具类，提供增强的Jackson集成
- 扩展工具函数系统的JSON处理能力，支持更复杂的序列化/反序列化场景
- 增强工具函数与AI模型集成时的JSON数据处理能力
...
**更新** 新增JsonUtils类作为核心JSON处理工具类，提供增强的Jackson集成能力，确保序列化/反序列化行为的一致性和可靠性。

## 项目结构
工具函数系统主要由以下层次构成：
- 工具函数层：以函数式接口实现具体工具能力，如天气查询、目录列表。
- JSON处理层：提供增强的JSON序列化/反序列化能力，基于Jackson和Spring容器管理。
...
subgraph "JSON处理层"
JU["JsonUtils<br/>增强JSON工具类"]
SU["SpringUtils<br/>Spring容器工具"]
end
subgraph "控制器与服务层"
...
- [JsonUtils.java:1-211](file://src/main/java/cn/boss/data/ai/framework/common/util/json/JsonUtils.java#L1-L211)
...
- JSON处理增强
  - JsonUtils类：提供基于Spring容器管理的ObjectMapper，确保序列化/反序列化行为一致性。
  - 支持多种序列化/反序列化场景：字符串、字节数组、Pretty Print格式、路径提取等。
  - 集成Hutool JSON工具，提供类型安全的JSON处理能力。
- 工具元数据管理
...
- [JsonUtils.java:21-211](file://src/main/java/cn/boss/data/ai/framework/common/util/json/JsonUtils.java#L21-L211)
...
## 架构总览
工具函数系统采用"函数式实现 + 增强JSON处理 + 元数据管理 + AI回调集成"的四层架构。工具函数通过Spring组件注解注册，服务层通过工具回调解析器完成名称到回调的映射，JSON处理层提供统一的序列化/反序列化能力，最终在构建聊天选项时注入工具回调，使模型在推理过程中可调用工具。
...
- [JsonUtils.java:39-118](file://src/main/java/cn/boss/data/ai/framework/common/util/json/JsonUtils.java#L39-L118)
...
### JsonUtils类设计与实现
- 设计理念
  - 基于Spring容器管理的ObjectMapper，确保序列化/反序列化行为一致性
  - 从估值后端适配而来，提供类型安全的JSON处理能力
  - 集成Hutool JSON工具，支持多种JSON处理场景
- 核心功能
  - 对象转JSON：支持字符串、字节数组、Pretty Print格式
  - JSON转对象：支持Class类型、Type类型、TypeReference类型
  - 数组处理：支持泛型数组的序列化/反序列化
  - 路径提取：支持从JSON字符串中提取特定路径的节点
  - 类型判断：支持JSON字符串的类型判断
- Jackson集成
  - 使用SpringUtils.getBean获取ObjectMapper实例
  - 支持复杂的类型信息处理，包括@JsonTypeInfo场景
  - 提供parseObject2方法解决CLASS类型信息缺失问题
...
+parseObject(text, type) T
+parseObject2(text, clazz) T
+parseObject(bytes, clazz) T
+parseObject(text, typeReference) T
+parseObjectQuietly(text, typeReference) T
+parseArray(text, clazz) T[]
+parseArray(text, path, clazz) T[]
+parseTree(text) JsonNode
+parseTree(bytes) JsonNode
+isJson(text) boolean
+isJsonObject(text) boolean
}
class SpringUtils {
+getBean(clazz) T
+getBean(name, clazz) T
}
JsonUtils --> SpringUtils : "获取ObjectMapper"
```
...
### JsonUtils在系统中的应用
- AiValuationClient中的使用
  - 使用isJson方法验证HTTP响应是否为JSON格式
  - 使用parseObject方法将JSON响应转换为类型安全的对象
  - 支持TypeReference的复杂类型处理
- 工具函数系统中的潜在应用
  - 工具函数参数的JSON序列化/反序列化
  - 工具回调解析器的JSON配置处理
  - AI模型参数的JSON格式转换
...
- [JsonUtils.java:21-211](file://src/main/java/cn/boss/data/ai/framework/common/util/json/JsonUtils.java#L21-L211)
- [SpringUtils.java:8-35](file://src/main/java/cn/boss/data/ai/framework/common/util/spring/SpringUtils.java#L8-L35)
- [AiValuationClient.java:39-52](file://src/main/java/cn/boss/data/ai/framework/ai/core/valuation/AiValuationClient.java#L39-L52)

## 依赖分析
- 组件耦合
  - 工具函数通过Spring组件注解注册，名称即工具回调名称，与服务层的工具回调解析器形成松耦合
  - 服务层依赖工具回调解析器与数据访问层，保持业务逻辑清晰
  - JsonUtils类依赖SpringUtils获取ObjectMapper，确保JSON处理的一致性
- 外部依赖
  - Spring AI工具回调与工具调用管理器
  - 多平台模型客户端（OpenAI、Azure OpenAI、Anthropic、Gemini等）
  - Jackson核心库与Hutool JSON工具
- 风险点
  - 工具名称唯一性：创建/更新时必须通过解析器校验名称有效性
  - 路径安全：目录列表工具仅在本地文件系统生效，需谨慎控制访问范围
  - JSON处理一致性：通过Spring容器管理ObjectMapper确保序列化/反序列化行为一致
...
**图表来源**
- [AiToolServiceImpl.java:32-69](file://src/main/java/cn/boss/data/ai/service/model/AiToolServiceImpl.java#L32-L69)
- [JsonUtils.java:30](file://src/main/java/cn/boss/data/ai/framework/common/util/json/JsonUtils.java#L30)
- [SpringUtils.java:18-20](file://src/main/java/cn/boss/data/ai/framework/common/util/spring/SpringUtils.java#L18-L20)
- [AiUtils.java:29-79](file://src/main/java/cn/boss/data/ai/util/AiUtils.java#L29-L79)
- [AiModelFactoryImpl.java:115-159](file://src/main/java/cn/boss/data/ai/framework/ai/core/model/AiModelFactoryImpl.java#L115-L159)
- [AiChatMessageServiceImpl.java:343-371](file://src/main/java/cn/boss/data/ai/service/chat/AiChatMessageServiceImpl.java#L343-L371)
...
- JSON处理优化
  - JsonUtils类使用静态ObjectMapper实例，避免重复创建开销
  - 通过Spring容器管理ObjectMapper，确保配置一致性
  - 支持字节数组直接序列化，减少中间字符串转换开销
- 服务层校验与解析
...
- JSON处理异常
  - 现象：JSON序列化/反序列化失败
  - 原因：ObjectMapper配置不一致或类型信息缺失
  - 处理：检查Spring容器中的ObjectMapper配置，使用JsonUtils提供的类型安全方法
...
- [JsonUtils.java:43-47](file://src/main/java/cn/boss/data/ai/framework/common/util/json/JsonUtils.java#L43-L47)

## 结论
工具函数系统通过函数式实现与元数据管理相结合，实现了工具能力的标准化与可扩展性。借助工具回调解析器与ChatOptions注入，系统能够灵活地与多平台模型集成，在对话中按需调用工具，提升AI应用的实际价值。

**更新** 新增的JsonUtils类作为核心JSON处理工具类，提供了基于Jackson的增强JSON处理能力，通过Spring容器管理ObjectMapper确保序列化/反序列化行为的一致性。该类不仅服务于当前的工具函数系统，也为未来的系统扩展提供了强大的JSON处理基础。

开发者可遵循本文档的接口规范与最佳实践，快速扩展新的工具函数并安全地接入AI对话流程。同时，JsonUtils类的引入为系统提供了更加健壮和一致的JSON处理能力，支持更复杂的序列化/反序列化场景。
...
- 注册机制
  ...
  - 确保组件扫描路径包含工具函数所在包
- JSON处理最佳实践
  - 使用JsonUtils进行JSON序列化/反序列化，确保类型安全
  - 对于复杂类型，优先使用TypeReference而非Class类型
  - 利用JsonUtils的路径提取功能处理嵌套JSON结构
- 与AI模型集成
...
- JsonUtils类通过Spring容器管理ObjectMapper，确保JSON处理的一致性
